...
function CashOutModal({ closeForCashout, isOpenForCashOut }) {
  ...
  const handleSendMoney = async (e) => {
    ...
    await axios
      .patch("http://localhost:3000/cashOut", CashOutInfo)
      .then((res) => {
        console.log(res.data);
        if (res.data.message === "Cash Out Successfully!") {
  ...
  return (
    <div>
      <Dialog
        open={isOpenForCashOut}
        as="div"
        className="relative z-10 focus:outline-none"
        onClose={closeForCashout}
      >
